			...
			<div class="caixa">
				...
				<form action="includes/usuario/deletar.inc.php">
					<div class="form-group select-caixa">
						<select class="lista-estoque chosen-select" onchange="clienteChange()" id="estoque" name="id" type="search">
							<?php
								$sql = "SELECT id, nome FROM usuarios WHERE id <> '$userid' ORDER BY nome;";

								if ($res = mysqli_query($conn, $sql)) {
									while ($row = mysqli_fetch_assoc($res)) {
										printf("<option value='%d'>%s</option>", $row['id'], $row['nome']);
									}
								}			
							?>
						</select>
				...
				<?php
					...
					if (isset($_GET['deletar'])) {
						$deletar = mysqli_real_escape_string($conn, $_GET['deletar']);
						...
						else if ($deletar == "erro") {
							echo "<div class='alert alert-danger'>
									  <strong>Erro!</strong> O usuário não foi deletado.
								  </div>";
						}
					}
				?>
